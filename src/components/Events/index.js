...
class Events extends Component {
  ...
  render() {
    const {status, activeEventId} = this.state
    return (
          ...
          <ul className="event-list-container">
            {eventsList.map(eachEvent => (
              <EventItem
                ...
                isActiveEvent={eachEvent.id === activeEventId}
              />
            ))}
          </ul>
        </div>
        <div className="event-status-container">
          <ActiveEventRegistrationDetails registrationStatus={status} />
        </div>
      </div>
    )
  }
